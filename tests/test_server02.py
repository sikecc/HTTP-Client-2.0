...
Behavior:
    This test case generates a response that does not include a chunked
    transfer-encoding or a content length and instead uses closing the socket
    to indicate an end of the body.  Other than the missing headers, this
    server generates a normal HTTP response, which includes headers that are
    not testing for any edge cases and a simple static body.

Tests:
    The HTTP standard includes many different ways for a server to specify the
    end of a response body.  This server tests whether or not the client
    correctly handles the response even if its length is not specified.

Notes:
    `curl -f <addr>` prints the correct body for this message to stdout.
    However, it also writes this complaint to stderr: 
        "curl: (56) Recv failure: Connection reset by peer"

    Similarly, Firefox is flakey as to whether or not it prints the body or
    complains that the connection was reset.
"""
