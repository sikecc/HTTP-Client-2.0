...
Behavior:
    This test case generates an HTTP response that has extra white-space in
    various parts of the HTTP headers.  Otherwise, the headers are normal, and
    the response contains a simple static body whose length is correctly
    specified with a Content-Length.

Tests:
    This server tests whether the client can corectly handle oddly formatted
    headers that contain extra and unnecessary whitespace.
